...
export default function PatientDashboard() {
  ...
  // 1. Run once on mount (Profile and Doctors list rarely change)
  useEffect(() => {
    fetchPatientProfile();
    fetchDoctors();
  }, []);

  // 2. Run every time 'activeTab' changes
  useEffect(() => {
    // Only refresh data if the user is looking at the Dashboard
    if (activeTab === "Dashboard") {
      console.log("Refreshing Dashboard Data..."); // Debug log
      fetchAppointments();
      fetchCareTeam();
      fetchPrescriptions();
    }
  }, [activeTab]); // <--- This dependency is the key!
  ...
  const fetchPatientProfile = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log("Patient Data Received:", data); // DEBUG: Check the structure here!

        // If your backend sends { patient: { ... } }, use data.patient
        const profileData = data.patient ? data.patient : data;
  ...
  const renderAppointmentBooking = () => {
    const handleInputChange = (e) => {
      ...
      // 1. Create a copy of what the data WILL look like
      const nextData = { ...appointmentData, [name]: value };

      // 2. Update the state
      setAppointmentData(nextData);

      // 3. Only fetch if both doctorId and date exist, using the fresh 'nextData'
      if (nextData.doctorId && nextData.date) {
        fetchAvailableSlots(nextData.doctorId, nextData.date);
      } else {
        // Clear slots if one is missing
        setAvailableSlots([]);
      }
    };
