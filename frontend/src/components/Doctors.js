...
export default function DoctorDashboard() {
  ...
  // FIX 1: Split useEffects. This one runs ONCE to get static profile data.
  useEffect(() => {
    fetchDoctorProfile();
  }, []);

  // FIX 1: This one runs EVERY TIME you switch to the Dashboard tab.
  // This ensures the counts and lists are always fresh.
  useEffect(() => {
  ...
  const fetchExistingPrescriptions = async (patientId) => {
    ...
    try {
      ...
      const response = await fetch(`/api/doctor/prescriptions/${patientId}`, {
        headers: {
  ...
  const fetchPatientsWithAppointments = async () => {
    try {
      ...
      const response = await fetch('/api/doctor/patients-with-appointments', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const renderPatientManagement = () => {
    ...
    const fetchAvailableSlots = async (patientId, date) => {
      ...
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`/api/doctor/available-slots?patientId=${patientId}&date=${date}`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
      ...
      if (selectedAction === 'prescribe-medication') {
        try {
          ...
          const url = appointmentData.prescriptionId
            ? `/api/doctor/prescriptions/${appointmentData.prescriptionId}`
          ...
          if (response.ok) {
            ...
            fetchExistingPrescriptions(appointmentData.patientId);
            setSelectedAction('');
            // FIX 3: Redirect to dashboard to update counts
