...
router.get('/patients-with-appointments', auth, async (req, res) => {
  try {
    ...
    const now = new Date(); // Capture the current time once

    // 1. Get all appointments for this doctor, sorted Oldest -> Newest
    ...
    const patientsWithAppointments = patients.map(patient => {
      // Get all appointments for this specific patient
      const patientAppointments = appointments.filter(app => app.patientId.toString() === patient._id.toString());

      // Filter: Separate Past and Future
      // Note: We use 'new Date(app.date)' to ensure we are comparing valid date objects
      const pastAppointments = patientAppointments.filter(app => new Date(app.date) < now);
      const futureAppointments = patientAppointments.filter(app => new Date(app.date) >= now);

      // LOGIC FIX:
      // lastVisit: In a sorted list (Ascending), the most recent past visit is the LAST item in the past array.
      const lastVisit = pastAppointments.length > 0 
        ? pastAppointments[pastAppointments.length - 1] 
        : null;

      // nextAppointment: In a sorted list (Ascending), the soonest future visit is the FIRST item in the future array.
